        ...
        await API.post("api", "/api/archive/query", data)
            .then((response) => {
                ...
                for (const header in response["ResultSet"]["Rows"][0]) {
                    for (const column in response["ResultSet"]["Rows"][0][header]) {
                        tableHeaders.push({
                            id: response["ResultSet"]["Rows"][0][header][column][
                                "VarCharValue"
                            ],
                            header:
                    ...
                    if (rows !== "0") {
                        for (const row in response["ResultSet"]["Rows"][rows]) {
                            ...
                            for (const keys in tableHeaders) {
                                ...
                                rowKeys[key] =
                                    response["ResultSet"]["Rows"][rows][row][keys][
                                    "VarCharValue"
                                    ];
                                rowKeys["description"] = "This is the first item";
                                rowKeys["type"] = "1A";
                                rowKeys["size"] = "Small";
                            }
        ...
        <SpaceBetween size="xl">
            ...
            <Table
                ...
                onSelectionChange={({ detail }) => {
                    ...
                    setSqlStatement(formalSqlStatement(detail.selectedItems as any));
                    setAceLoading(false);
                }}
                empty={
                    <Box textAlign="center" color="inherit">
